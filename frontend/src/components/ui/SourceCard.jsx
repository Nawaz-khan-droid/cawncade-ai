...
export default function SourceCard({ src }) {
  ...
  if (src.stance === "CONTRADICTS" || lowerSnippet.includes("debunk") || lowerSnippet.includes("fake") || lowerSnippet.includes("false") || lowerSnippet.includes("untrue")) {
    ...
    stateConfig = {
      ...
      badgeClass: "bg-slate-200 dark:bg-slate-700/50 text-slate-800 dark:text-slate-300 border-slate-300 dark:border-slate-600",
      icon: HelpCircle,
    ...
    >
      <div className="flex items-center justify-between gap-2 min-w-0">
        ...
        <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded shrink-0 ${
          src.is_trusted ? 'bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-400'
        }`}>
          {src.is_trusted ? '✓ TRUSTED' : src.trust_tier || 'NEWS'}
        </span>
      </div>

      <p className="text-xs font-semibold text-slate-900 dark:text-slate-200 line-clamp-2 leading-snug break-words">{src.title}</p>
