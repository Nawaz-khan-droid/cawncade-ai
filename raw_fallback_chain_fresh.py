...
# Ensure logger captures all INFO and WARNING logs
logging.basicConfig(level=logging.INFO, format="%(asctime)s | %(levelname)s | %(name)s | %(message)s")

print("=================== EXECUTING FRESH FALLBACK CHAIN TRACE (UNCACHED) ===================")
with TestClient(app) as client:
    payload = {
        "input_text": f"James Webb Space Telescope detects water vapor on exoplanet WASP-96b",
        "input_type": "text"
    }
